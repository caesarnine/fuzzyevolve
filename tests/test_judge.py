...
def test_validate_rankings_strict_permutation():
    ...
    rankings = [
        MetricRanking(metric="m1", ranked_tiers=[[0], [1]]),
        MetricRanking(metric="m2", ranked_tiers=[[0]]),
    ]
...
def test_validate_rankings_allows_ties():
    judge = LLMJudge("mock", ["m1", "m2"], rng=random.Random(0))
    rankings = [
        MetricRanking(metric="m1", ranked_tiers=[[0, 1]]),
        MetricRanking(metric="m2", ranked_tiers=[[0], [1]]),
    ]
    ranked_map, error = judge._validate_rankings(rankings, total_players=2)
    assert error is None
    assert ranked_map == {"m1": [[0, 1]], "m2": [[0], [1]]}
...
def test_rank_and_rate_repair_success():
    ...
    invalid = JudgeOutput(rankings=[MetricRanking(metric="m1", ranked_tiers=[[0]])])
    valid = JudgeOutput(
        rankings=[MetricRanking(metric="m1", ranked_tiers=[[0], [1]])]
    )
    calls = {"count": 0}
